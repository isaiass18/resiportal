...
/* Ficha integral de inmueble, mascotas y carga universal (2026-08-14). */
function resumenEspaciosInmueble(valor) {
    const espacios = String(valor || '').split('||').filter(Boolean).map(item => {
        const [codigo, clase] = item.split('|');
        return { codigo, clase };
    });
    if (!espacios.length) return '<span class="muted">Sin asignaciones</span>';
    return espacios.map(espacio => `<span class="inmueble-space-chip"><i class="fa-solid ${iconoClaseEspacio(espacio.clase)}"></i>${escapeHtml(espacio.codigo)} <small>${escapeHtml(espacio.clase === 'bodega' ? 'Bodega' : espacio.clase === 'moto' ? 'Moto' : 'Carro')}</small></span>`).join('');
}

function tarjetaEspacioInmueble(espacio) {
    return `<article class="inmueble-space-card"><i class="fa-solid ${iconoClaseEspacio(espacio.clase_espacio)}"></i><div><strong>${escapeHtml(espacio.codigo)}</strong><small>${escapeHtml(etiquetaClaseEspacio(espacio.clase_espacio))}</small><small>${escapeHtml(etiquetaUbicacionEspacio(espacio))}</small></div></article>`;
}
...
loadInmuebles = async function () {
    ...
    const tabla = document.getElementById('tb-inmuebles');
    const cabecera = tabla?.closest('table')?.querySelector('thead tr');
    if (cabecera) cabecera.innerHTML = '<th>Unidad</th><th>Torre/bloque</th><th>Ocupación</th><th>Espacios asignados</th><th>Vehículos / mascotas</th><th>Mora</th><th>Acciones</th>';
    if (!tabla) return;
    tabla.innerHTML = inmueblesActuales.length ? inmueblesActuales.map(inmueble => {
        const sinCuenta = Number(inmueble.num_cuentas || 0) === 0;
        const estado = `<span class="inmueble-state ${sinCuenta ? 'is-empty' : 'is-linked'}"><i class="fa-solid ${sinCuenta ? 'fa-user-slash' : 'fa-user-check'}"></i> ${sinCuenta ? 'Sin usuario con acceso' : `${Number(inmueble.num_cuentas)} con acceso`}</span><small>${Number(inmueble.num_residentes)} residente(s) · ${Number(inmueble.num_personas)} persona(s)</small>`;
        return `<tr class="inmueble-row" tabindex="0" onclick="window.abrirDetalleInmueble(${Number(inmueble.id)})"><td><strong>${escapeHtml(inmueble.nomenclatura || inmueble.apartamento)}</strong><br><small>${escapeHtml(inmueble.tipo_unidad)}</small></td><td>${escapeHtml(inmueble.torre || '—')}</td><td>${estado}</td><td><div class="inmueble-space-chips">${resumenEspaciosInmueble(inmueble.espacios_asignados)}</div></td><td>${Number(inmueble.num_vehiculos)} vehículo(s)<br><small>${Number(inmueble.num_mascotas)} mascota(s)</small></td><td>${formatCurrency(inmueble.mora_actual)}</td><td class="inmueble-actions"><button class="btn btn-ghost" onclick="event.stopPropagation(); window.abrirDetalleInmueble(${Number(inmueble.id)})">Ver ficha</button><button class="btn btn-ghost" onclick="event.stopPropagation(); window.abrirModalInmueble(${Number(inmueble.id)})">Editar unidad</button></td></tr>`;
    }).join('') : '<tr><td colspan="7">No hay unidades registradas.</td></tr>';
    document.getElementById('formInmueble').onsubmit = guardarInmueble;
};
...
async function abrirEditorActivo(tipo, activo, inmuebleId) {
    ...
    if (!modal) { document.body.insertAdjacentHTML('beforeend', '<div id="modalEditarActivo" class="login-modal hidden"></div>'); modal = document.getElementById('modalEditarActivo'); }
    const esVehiculo = tipo === 'vehiculo';
    const camposMascota = `<label>Tipo<input name="tipo" maxlength="50" list="tiposMascota" value="${escapeHtml(activo.tipo || '')}" placeholder="Ej.: Perro" required></label><datalist id="tiposMascota"><option value="Perro"><option value="Gato"><option value="Ave"><option value="Pez"><option value="Conejo"><option value="Otro"></datalist><label>Nombre<input name="nombre" maxlength="100" value="${escapeHtml(activo.nombre || '')}" required></label><label>Raza<input name="raza" maxlength="100" value="${escapeHtml(activo.raza || '')}" placeholder="Opcional"></label><label>Descripción<textarea name="descripcion" maxlength="5000" rows="3" placeholder="Color, señales particulares u observaciones">${escapeHtml(activo.descripcion || '')}</textarea></label>`;
    modal.innerHTML = `<div class="login-box asset-editor-dialog"><button class="close-btn" type="button" onclick="document.getElementById('modalEditarActivo').classList.add('hidden')"><i class="fa-solid fa-xmark"></i></button><h2>Editar ${esVehiculo ? 'vehículo' : 'mascota'}</h2><form class="stack-form">${esVehiculo ? `<input name="placa" value="${escapeHtml(activo.placa)}" required><input name="tipo" value="${escapeHtml(activo.tipo || '')}" required><input name="marca" value="${escapeHtml(activo.marca || '')}" placeholder="Marca"><input name="linea" value="${escapeHtml(activo.linea || '')}" placeholder="Línea">` : camposMascota}<button class="btn btn-primary" type="submit">Guardar cambios</button></form></div>`;
    modal.querySelector('form').onsubmit = async evento => { evento.preventDefault(); const datos = new FormData(evento.currentTarget); datos.append('action', `actualizar_${tipo}`); datos.append(`${tipo}_id`, activo.id); const respuesta = await fetch('api/inmuebles.php', { method: 'POST', body: datos }); const resultado = await respuesta.json(); window.notificar(resultado.message, resultado.status === 'success' ? 'success' : 'error'); if (resultado.status === 'success') { modal.classList.add('hidden'); if (currentUser?.rol === 'admin') window.abrirDetalleInmueble(inmuebleId); else loadHomeResidente(); } };
    modal.classList.remove('hidden');
}
...
window.abrirDetalleInmueble = async function (inmuebleId) {
    ...
    const [detalle, usuarios] = await Promise.all([detalleR.json(), usuariosR.json()]);
    if (detalle.status !== 'success') return window.notificar(detalle.message || 'No fue posible abrir el inmueble.', 'error');
    const { inmueble, espacios = [], personas = [], vehiculos = [], mascotas = [] } = detalle.data;
    window.detalleInmuebleActual = { inmueble, espacios, personas };
    let modal = document.getElementById('modalDetalleInmueble');
    if (!modal) { document.body.insertAdjacentHTML('beforeend', '<div id="modalDetalleInmueble" class="login-modal detalle-inmueble-modal hidden"></div>'); modal = document.getElementById('modalDetalleInmueble'); }
    const espaciosHtml = espacios.length ? espacios.map(tarjetaEspacioInmueble).join('') : '<p class="muted">Esta unidad no tiene parqueaderos ni bodegas asignados.</p>';
    const personasHtml = personas.length ? personas.map(persona => `<article class="inmueble-person-card"><span class="inmueble-person-icon"><i class="fa-solid ${persona.tipo_relacion === 'propietario' ? 'fa-key' : 'fa-house-user'}"></i></span><div class="inmueble-person-info"><strong>${escapeHtml(persona.nombre)}</strong><small>${escapeHtml(persona.tipo_relacion)} · ${Number(persona.tiene_cuenta) ? 'Acceso al portal' : 'Solo directorio'}</small><small>${escapeHtml(persona.contacto || persona.email || 'Sin contacto')}</small></div><div class="inmueble-person-actions"><button class="btn btn-ghost" type="button" onclick="window.editarPersonaVinculada(${Number(persona.id)}, '${escapeHtml(persona.tipo_relacion)}', ${Number(inmueble.id)})">Editar datos</button><button class="btn btn-ghost" type="button" onclick="window.editarVinculoInmueble(${Number(persona.relacion_id)})">Cambiar unidad</button><button class="btn btn-ghost danger-action" type="button" onclick="window.desvincularPersonaInmueble(${Number(persona.relacion_id)}, ${Number(inmueble.id)})">Desvincular</button></div></article>`).join('') : '<p class="empty-state">No hay personas vinculadas.</p>';
    const opcionesPersonas = usuarios.status === 'success' ? (usuarios.data || []).filter(persona => ['residente', 'propietario'].includes(persona.rol)).map(persona => `<option value="${Number(persona.id)}">${escapeHtml(persona.nombre)} · ${escapeHtml(persona.documento)}</option>`).join('') : '';
    const activosHtml = (tipo, lista) => lista.length ? `<div class="asset-list">${lista.map(activo => { const esVehiculo = tipo === 'vehiculo'; const titulo = esVehiculo ? activo.placa : (activo.nombre || activo.descripcion || 'Mascota sin nombre'); const detalleActivo = esVehiculo ? [activo.tipo, activo.marca, activo.linea].filter(Boolean).join(' · ') : [activo.tipo, activo.raza, activo.descripcion].filter(Boolean).join(' · '); return `<article><div><strong>${escapeHtml(titulo)}</strong><small>${escapeHtml(detalleActivo || 'Sin detalles adicionales')}</small></div><div class="asset-actions"><button class="btn btn-ghost" type="button" onclick='abrirEditorActivo("${tipo}", ${JSON.stringify(activo).replace(/'/g, '&#39;')}, ${Number(inmueble.id)})'>Editar</button><button class="btn btn-ghost danger-action" type="button" onclick="window.eliminarActivoInmueble('${tipo}', ${Number(activo.id)}, ${Number(inmueble.id)})">Eliminar</button></div></article>`; }).join('')}</div>` : '<p class="muted">Sin registros.</p>';
    modal.innerHTML = `<div class="login-box inmueble-detail-dialog"><button class="close-btn" type="button" onclick="document.getElementById('modalDetalleInmueble').classList.add('hidden')"><i class="fa-solid fa-xmark"></i></button><header class="inmueble-detail-heading"><div><p class="section-kicker">Ficha integral del inmueble</p><h2>${escapeHtml(inmueble.nomenclatura || inmueble.apartamento)}</h2><p>${escapeHtml(inmueble.tipo_unidad)} · ${escapeHtml(inmueble.torre || 'Sin torre')}</p></div><div class="inmueble-summary-actions"><strong>${formatCurrency(inmueble.mora_actual)}</strong><button class="btn btn-ghost" type="button" onclick="window.verHistorialInmueble(${Number(inmueble.id)})"><i class="fa-solid fa-clock-rotate-left"></i> Historial de espacios</button></div></header><section class="inmueble-detail-section"><div class="inmueble-section-title"><div><h3><i class="fa-solid fa-square-parking"></i> Parqueaderos y bodegas asignados</h3><small>La gestión de asignaciones se realiza en Espacios y bodegas.</small></div></div><div class="inmueble-space-grid">${espaciosHtml}</div></section><section class="inmueble-detail-section"><div class="inmueble-section-title"><h3>Personas vinculadas</h3><button class="btn btn-primary" type="button" onclick="window.abrirPersonaDesdeInmueble(${Number(inmueble.id)})"><i class="fa-solid fa-user-plus"></i> Agregar persona</button></div><div class="inmueble-person-grid">${personasHtml}</div><form id="formVincularUsuarioInmueble" class="inmueble-link-form"><input type="hidden" name="inmueble_id" value="${Number(inmueble.id)}"><label>Persona<select name="usuario_id" required><option value="">Selecciona una persona…</option>${opcionesPersonas}</select></label><label>Relación<select name="tipo_relacion" required><option value="residente">Residente</option><option value="propietario">Propietario</option></select></label><button class="btn btn-ghost" type="submit">Vincular</button></form></section><section class="inmueble-detail-grid"><div class="inmueble-detail-section"><h3><i class="fa-solid fa-car"></i> Vehículos</h3>${activosHtml('vehiculo', vehiculos)}</div><div class="inmueble-detail-section"><h3><i class="fa-solid fa-paw"></i> Mascotas</h3>${activosHtml('mascota', mascotas)}</div></section></div>`;
    modal.querySelector('#formVincularUsuarioInmueble').onsubmit = async evento => { evento.preventDefault(); const datos = new FormData(evento.currentTarget); datos.append('action', 'vincular_usuario'); const respuesta = await fetch('api/inmuebles.php', { method: 'POST', body: datos }); const resultado = await respuesta.json(); window.notificar(resultado.message, resultado.status === 'success' ? 'success' : 'error'); if (resultado.status === 'success') { loadInmuebles(); window.abrirDetalleInmueble(inmuebleId); } };
    modal.classList.remove('hidden');
};

window.editarVinculoInmueble = async function (relacionId) {
    const detalle = window.detalleInmuebleActual;
    const persona = detalle?.personas?.find(item => Number(item.relacion_id) === Number(relacionId));
    if (!persona) return window.notificar('No fue posible identificar el vínculo.', 'error');
    if (!inmueblesActuales?.length) await loadInmuebles();
    let modal = document.getElementById('modalEditarVinculo');
    if (!modal) { document.body.insertAdjacentHTML('beforeend', '<div id="modalEditarVinculo" class="login-modal hidden"></div>'); modal = document.getElementById('modalEditarVinculo'); }
    const opciones = (inmueblesActuales || []).map(item => `<option value="${Number(item.id)}" ${Number(item.id) === Number(detalle.inmueble.id) ? 'selected' : ''}>${escapeHtml([item.torre, item.nomenclatura || item.apartamento].filter(Boolean).join(' · '))}</option>`).join('');
    modal.innerHTML = `<div class="login-box inmueble-person-dialog"><button class="close-btn" type="button" onclick="document.getElementById('modalEditarVinculo').classList.add('hidden')"><i class="fa-solid fa-xmark"></i></button><h2>Cambiar unidad vinculada</h2><p class="muted">Moverás a <strong>${escapeHtml(persona.nombre)}</strong> sin borrar su ficha ni su acceso al portal.</p><form class="stack-form"><input type="hidden" name="relacion_id" value="${Number(relacionId)}"><input type="hidden" name="tipo_relacion" value="${escapeHtml(persona.tipo_relacion)}"><label>Unidad<select name="inmueble_id" required>${opciones}</select></label><button class="btn btn-primary" type="submit">Guardar cambio</button></form></div>`;
    modal.querySelector('form').onsubmit = async evento => { evento.preventDefault(); const datos = new FormData(evento.currentTarget); datos.append('action', 'actualizar_vinculo'); const respuesta = await fetch('api/inmuebles.php', { method: 'POST', body: datos }); const resultado = await respuesta.json(); window.notificar(resultado.message, resultado.status === 'success' ? 'success' : 'error'); if (resultado.status === 'success') { modal.classList.add('hidden'); loadInmuebles(); window.abrirDetalleInmueble(Number(datos.get('inmueble_id'))); } };
    modal.classList.remove('hidden');
};

window.desvincularPersonaInmueble = async function (relacionId, inmuebleId) {
    if (!await confirmarAccion({ titulo: '¿Desvincular persona?', texto: 'Se retirará solo de esta unidad. La persona, su historial y su acceso al portal no se eliminarán.', confirmar: 'Desvincular', icono: 'warning' })) return;
    const datos = new FormData(); datos.append('action', 'desvincular_usuario'); datos.append('relacion_id', relacionId);
    const respuesta = await fetch('api/inmuebles.php', { method: 'POST', body: datos }); const resultado = await respuesta.json();
    window.notificar(resultado.message, resultado.status === 'success' ? 'success' : 'error');
    if (resultado.status === 'success') { loadInmuebles(); window.abrirDetalleInmueble(inmuebleId); }
};

function prepararFormularioActivos(inmuebles) {
    const opciones = inmuebles.map(item => `<option value="${Number(item.id)}">${escapeHtml([item.torre, item.nomenclatura || item.apartamento].filter(Boolean).join(' · '))}</option>`).join('');
    const vehiculos = document.getElementById('formNuevoVehiculo');
    const mascotas = document.getElementById('formNuevaMascota');
    if (vehiculos) vehiculos.innerHTML = `<select name="inmueble_id" class="input" required><option value="">Unidad asociada…</option>${opciones}</select><input type="text" name="placa" class="input mt-2" placeholder="Placa" required><select name="tipo" class="input mt-2" required><option value="">Tipo...</option><option value="Carro">Carro</option><option value="Moto">Moto</option><option value="Bicicleta">Bicicleta</option></select><input type="text" name="marca" class="input mt-2" placeholder="Marca (opcional)"><input type="text" name="linea" class="input mt-2" placeholder="Línea (opcional)"><button type="submit" class="btn btn-primary mt-4" style="width:100%">Guardar</button>`;
    if (mascotas) mascotas.innerHTML = `<select name="inmueble_id" class="input" required><option value="">Unidad asociada…</option>${opciones}</select><input name="tipo" class="input mt-2" maxlength="50" list="tiposMascotaInicio" placeholder="Tipo: perro, gato, ave..." required><datalist id="tiposMascotaInicio"><option value="Perro"><option value="Gato"><option value="Ave"><option value="Pez"><option value="Conejo"><option value="Otro"></datalist><input name="nombre" class="input mt-2" maxlength="100" placeholder="Nombre de la mascota" required><input name="raza" class="input mt-2" maxlength="100" placeholder="Raza (opcional)"><textarea name="descripcion" class="input mt-2" maxlength="5000" placeholder="Descripción u observaciones (opcional)" rows="2"></textarea><button type="submit" class="btn btn-primary mt-4" style="width:100%">Guardar mascota</button>`;
}

const cargarInicioIntegralBase = loadHomeResidente;
loadHomeResidente = async function () {
    await cargarInicioIntegralBase();
    if (!['residente', 'propietario'].includes(currentUser?.rol)) return;
    const [unidadesR, vehiculosR, mascotasR] = await Promise.all([fetch('api/inmuebles.php?action=mis_inmuebles'), fetch('api/inmuebles.php?action=mis_vehiculos'), fetch('api/inmuebles.php?action=mis_mascotas')]);
    const [unidades, vehiculos, mascotas] = await Promise.all([unidadesR.json(), vehiculosR.json(), mascotasR.json()]);
    const vista = document.querySelector('#view-container .view');
    if (unidades.status === 'success') {
        prepararFormularioActivos(unidades.data || []);
        vista?.querySelector('#misEspaciosAsignados')?.remove();
        vista?.querySelector('#misUnidadesAsignadas')?.remove();
        const tarjetas = unidades.data?.length ? unidades.data.map(unidad => `<article class="mi-unidad-card"><header><i class="fa-solid fa-building"></i><div><strong>${escapeHtml(unidad.nomenclatura || unidad.apartamento)}</strong><small>${escapeHtml([unidad.tipo_unidad, unidad.torre].filter(Boolean).join(' · '))}</small></div></header><div class="mi-unidad-espacios">${unidad.espacios?.length ? unidad.espacios.map(tarjetaEspacioInmueble).join('') : '<small>Sin parqueadero ni bodega asignados.</small>'}</div></article>`).join('') : '<p class="muted">No tienes una unidad vinculada actualmente.</p>';
        vista?.insertAdjacentHTML('afterbegin', `<section id="misUnidadesAsignadas" class="card mis-unidades-card"><div class="section-toolbar"><div><p class="section-kicker">Mi residencia</p><h3>Mi apartamento/casa, parqueaderos y bodegas</h3><p class="muted">Estos son los espacios asignados actualmente a tus unidades.</p></div></div><div class="mis-unidades-grid">${tarjetas}</div></section>`);
    }
    const tablaVehiculos = document.getElementById('tb-mis-vehiculos');
    if (tablaVehiculos && vehiculos.status === 'success') {
        const cabecera = tablaVehiculos.closest('table')?.querySelector('thead tr'); if (cabecera) cabecera.innerHTML = '<th>Placa</th><th>Tipo</th><th>Unidad</th><th>Acciones</th>';
        tablaVehiculos.innerHTML = vehiculos.data?.length ? vehiculos.data.map(item => `<tr><td><strong>${escapeHtml(item.placa)}</strong></td><td>${escapeHtml([item.tipo, item.marca, item.linea].filter(Boolean).join(' · '))}</td><td>${escapeHtml([item.torre, item.nomenclatura || item.apartamento].filter(Boolean).join(' · '))}</td><td class="asset-actions"><button class="btn btn-ghost" onclick='abrirEditorActivo("vehiculo", ${JSON.stringify(item).replace(/'/g, '&#39;')}, ${Number(item.inmueble_id)})'>Editar</button><button class="btn btn-ghost danger-action" onclick="window.eliminarActivoInmueble('vehiculo', ${Number(item.id)}, 0)">Eliminar</button></td></tr>`).join('') : '<tr><td colspan="4">Aún no tienes vehículos registrados.</td></tr>';
    }
    const tablaMascotas = document.getElementById('tb-mis-mascotas');
    if (tablaMascotas && mascotas.status === 'success') {
        const cabecera = tablaMascotas.closest('table')?.querySelector('thead tr'); if (cabecera) cabecera.innerHTML = '<th>Mascota</th><th>Tipo / raza</th><th>Unidad</th><th>Descripción</th><th>Acciones</th>';
        tablaMascotas.innerHTML = mascotas.data?.length ? mascotas.data.map(item => `<tr><td><strong>${escapeHtml(item.nombre || 'Sin nombre')}</strong></td><td>${escapeHtml([item.tipo, item.raza].filter(Boolean).join(' · ') || '—')}</td><td>${escapeHtml([item.torre, item.nomenclatura || item.apartamento].filter(Boolean).join(' · '))}</td><td>${escapeHtml(item.descripcion || '—')}</td><td class="asset-actions"><button class="btn btn-ghost" onclick='abrirEditorActivo("mascota", ${JSON.stringify(item).replace(/'/g, '&#39;')}, ${Number(item.inmueble_id)})'>Editar</button><button class="btn btn-ghost danger-action" onclick="window.eliminarActivoInmueble('mascota', ${Number(item.id)}, 0)">Eliminar</button></td></tr>`).join('') : '<tr><td colspan="5">Aún no tienes mascotas registradas.</td></tr>';
    }
};

function normalizarImportacion(valor) { return String(valor || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]/g, ''); }
window.initImportView = function () {
    const input = document.getElementById('excelFile'); const zona = document.getElementById('uploadZone'); const seccion = document.getElementById('mappingSection'); const formulario = document.getElementById('mappingForm');
    if (!input || !zona || !seccion || !formulario || zona.dataset.importacionUniversal) return;
    zona.dataset.importacionUniversal = '1';
    ...
    const tipos = {
        residentes: { titulo: 'Residentes', descripcion: 'Tú eliges las columnas de documento, nombre, correo, teléfono y unidad.', campos: [['documento', 'Documento *'], ['nombre', 'Nombre completo *'], ['email', 'Correo'], ['contacto', 'Teléfono'], ['inmueble_nomenclatura', 'Nomenclatura del inmueble']] },
        propietarios: { titulo: 'Propietarios', descripcion: 'Tú eliges las columnas de documento, nombre, correo, teléfono y unidad.', campos: [['documento', 'Documento *'], ['nombre', 'Nombre completo *'], ['email', 'Correo'], ['contacto', 'Teléfono'], ['inmueble_nomenclatura', 'Nomenclatura del inmueble']] },
        inmuebles: { titulo: 'Apartamentos o casas', descripcion: 'Mapea nomenclatura y los datos disponibles de tu archivo.', campos: [['nomenclatura', 'Nomenclatura *'], ['tipo_unidad', 'Tipo: apartamento/casa'], ['torre', 'Torre o bloque'], ['apartamento', 'Apartamento'], ['coeficiente', 'Coeficiente'], ['cuota_administracion', 'Cuota de administración'], ['mora_actual', 'Mora actual']] },
        parqueaderos: { titulo: 'Parqueaderos, motos y bodegas', descripcion: 'Mapea el código y opcionalmente la clase física y unidad asignada.', campos: [['codigo', 'Código *'], ['tipo', 'Régimen: privado/administracion/visitante/otro'], ['clase_espacio', 'Clase: carro/moto/bodega'], ['sotano', 'Sótano o nivel'], ['ubicacion', 'Ubicación'], ['estado', 'Estado: disponible/asignado/inactivo'], ['observaciones', 'Observaciones'], ['inmueble_nomenclatura', 'Nomenclatura asignada']] },
        mascotas: { titulo: 'Mascotas', descripcion: 'Incluye la unidad, tipo, nombre, raza y descripción si están disponibles.', campos: [['inmueble_nomenclatura', 'Nomenclatura del inmueble *'], ['tipo_mascota', 'Tipo *'], ['nombre_mascota', 'Nombre *'], ['raza', 'Raza'], ['descripcion', 'Descripción']] }
    };
    zona.insertAdjacentHTML('beforebegin', `<p class="import-help">1. Selecciona la información a cargar. 2. Sube un XLSX o CSV, sin importar cómo se llamen sus columnas. 3. Indica qué columna corresponde a cada dato. 4. Valida la vista previa antes de importar.</p><div class="import-type-picker">${Object.entries(tipos).map(([clave, tipo], indice) => `<label><input type="radio" name="importTipo" value="${clave}" ${indice === 0 ? 'checked' : ''}><strong>${tipo.titulo}</strong><small>${tipo.descripcion}</small></label>`).join('')}</div><div id="importPreview"></div>`);
    let headers = []; let listo = false;
    const seleccionado = () => document.querySelector('input[name="importTipo"]:checked')?.value || 'residentes';
    const sinonimos = { documento: ['documento', 'cedula', 'identificacion', 'id'], nombre: ['nombre', 'nombrecompleto', 'nombres'], email: ['email', 'correo', 'mail'], contacto: ['telefono', 'celular', 'contacto'], inmueble_nomenclatura: ['inmueble', 'nomenclatura', 'apartamento', 'apto', 'unidad', 'casa'], tipo_mascota: ['tipo', 'especie', 'tipomascota'], nombre_mascota: ['nombre', 'mascota', 'nombremascota'], tipo_unidad: ['tipounidad', 'tipo'], cuota_administracion: ['cuota', 'administracion'], mora_actual: ['mora', 'deuda'] };
    const opciones = campo => `<option value="">— No mapear —</option>${headers.map((header, indice) => { const normalizado = normalizarImportacion(header); const sugerido = (sinonimos[campo] || [campo]).map(normalizarImportacion).includes(normalizado) || normalizado.includes(normalizarImportacion(campo)); return `<option value="${indice}" ${sugerido ? 'selected' : ''}>${escapeHtml(header || `Columna ${indice + 1}`)}</option>`; }).join('')}`;
    const mapeo = () => Object.fromEntries(tipos[seleccionado()].campos.map(([campo]) => [campo, formulario.querySelector(`[name="map_${campo}"]`)?.value || '']));
    const enviar = async accion => { const archivo = input.files[0]; if (!archivo) return { status: 'error', message: 'Selecciona un archivo XLSX o CSV primero.' }; const datos = new FormData(); datos.append('action', accion); datos.append('tipo', seleccionado()); datos.append('mapping', JSON.stringify(mapeo())); datos.append('file', archivo); return leerRespuestaImportacion(await fetch('api/import.php', { method: 'POST', body: datos })); };
    input.onchange = async () => { const archivo = input.files[0]; if (!archivo) return; const datos = new FormData(); datos.append('action', 'get_headers'); datos.append('tipo', seleccionado()); datos.append('file', archivo); const resultado = await leerRespuestaImportacion(await fetch('api/import.php', { method: 'POST', body: datos })); if (resultado.status !== 'success') return window.notificar(resultado.message, 'error'); headers = resultado.data.headers; listo = false; formulario.innerHTML = tipos[seleccionado()].campos.map(([campo, etiqueta]) => `<div class="mapping-item"><label>${etiqueta}</label><select name="map_${campo}">${opciones(campo)}</select></div>`).join(''); seccion.classList.remove('hidden'); seccion.querySelector('h3').textContent = `Asignar columnas: ${tipos[seleccionado()].titulo}`; seccion.querySelector('p').textContent = `${resultado.data.rows} fila(s) detectadas. Los campos con * son obligatorios.`; formulario.insertAdjacentHTML('afterend', '<div class="actions mt-4" id="accionesMapeo"><button type="button" class="btn btn-ghost" id="btnPrevisualizarImportacion">Validar y ver vista previa</button></div>'); document.getElementById('btnProcesarImportacion').disabled = true; document.getElementById('btnPrevisualizarImportacion').onclick = async () => { const previo = await enviar('preview'); const panel = document.getElementById('importPreview'); if (previo.status !== 'success') return window.notificar(previo.message, 'error'); const info = previo.data; listo = info.summary.errores === 0 && info.summary.validas > 0; panel.innerHTML = `<p class="${listo ? 'import-ok' : 'import-errors'}">${listo ? `Listo: ${info.summary.validas} fila(s) válidas.` : `${info.summary.errores} error(es). Corrígelos antes de importar.`}</p>${info.sample?.length ? `<div class="import-preview-wrap"><table class="import-preview-table"><thead><tr>${Object.keys(info.sample[0]).filter(campo => campo !== '_fila').map(campo => `<th>${escapeHtml(campo)}</th>`).join('')}</tr></thead><tbody>${info.sample.map(fila => `<tr>${Object.keys(fila).filter(campo => campo !== '_fila').map(campo => `<td>${escapeHtml(fila[campo] || '—')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>` : ''}${info.errors?.length ? `<div class="import-errors">${info.errors.map(error => `Fila ${error.fila}: ${escapeHtml(error.mensaje)}`).join('<br>')}</div>` : ''}`; document.getElementById('btnProcesarImportacion').disabled = !listo; }; };
    document.querySelectorAll('input[name="importTipo"]').forEach(radio => radio.onchange = () => { input.value = ''; headers = []; listo = false; seccion.classList.add('hidden'); document.getElementById('importPreview').innerHTML = ''; document.getElementById('accionesMapeo')?.remove(); document.getElementById('btnProcesarImportacion').disabled = true; });
    document.getElementById('btnProcesarImportacion').onclick = async () => { if (!listo) return window.notificar('Valida la vista previa sin errores antes de importar.', 'warning'); if (!await confirmarAccion({ titulo: '¿Importar datos?', texto: 'Se crearán o actualizarán únicamente las filas validadas y el mapeo seleccionado.', confirmar: 'Importar datos', icono: 'question' })) return; const boton = document.getElementById('btnProcesarImportacion'); boton.disabled = true; const original = boton.textContent; boton.textContent = 'Importando…'; try { const resultado = await enviar('process'); window.notificar(resultado.message, resultado.status === 'success' ? 'success' : 'error'); if (resultado.status === 'success') { input.value = ''; seccion.classList.add('hidden'); document.getElementById('importPreview').innerHTML = ''; document.getElementById('accionesMapeo')?.remove(); } } finally { boton.textContent = original; boton.disabled = false; } };
};

window.aplicarMarcaPortal = async function () {
    try {
        const resultado = await (await fetch('api/conjuntos.php?action=public_config', { cache: 'no-store' })).json();
        if (resultado.status !== 'success') return;
        const marca = resultado.data || {}; const nombre = String(marca.nombre || 'ResiPortal'); const logo = String(marca.logo_url || '');
        document.title = `${nombre} · Portal residencial`;
        const topbar = document.querySelector('.topbar > div:first-child');
        if (topbar && !topbar.querySelector('[data-conjunto-brand]')) topbar.insertAdjacentHTML('afterbegin', '<div class="topbar-brand" data-conjunto-brand></div>');
        document.querySelectorAll('.public-nav .logo, .sidebar .logo, [data-conjunto-brand]').forEach(contenedor => { contenedor.innerHTML = logo ? `<img class="conjunto-brand-logo" src="${escapeHtml(logo)}" alt="Logo de ${escapeHtml(nombre)}"><span>${escapeHtml(nombre)}</span>` : `<i class="fa-solid fa-building-user" aria-hidden="true"></i><span>${escapeHtml(nombre)}</span>`; });
        document.querySelectorAll('.page-title').forEach(titulo => { if (/Bienvenido a ResiPortal/i.test(titulo.textContent)) titulo.textContent = `Bienvenido a ${nombre}`; });
        let favicon = document.getElementById('app-favicon'); if (!favicon) { favicon = document.createElement('link'); favicon.id = 'app-favicon'; favicon.rel = 'icon'; document.head.appendChild(favicon); } favicon.href = logo || favicon.href;
    } catch (error) { console.warn('No fue posible cargar la marca del conjunto.', error); }
};
const cargarVistaConMarcaCompletaBase = loadView;
loadView = function (vista) { const resultado = cargarVistaConMarcaCompletaBase(vista); Promise.resolve(resultado).finally(() => window.aplicarMarcaPortal()); return resultado; };


window.editarPersonaVinculada = async function (usuarioId, rolPersona, inmuebleId) {
    if (typeof window.openUsuarioModal !== 'function') return window.notificar('Abre el módulo Usuarios para editar esta persona.', 'warning');
    if (!Array.isArray(window.usuariosActuales) || !window.usuariosActuales.some(item => Number(item.id) === Number(usuarioId))) await loadUsuarios();
    window.openUsuarioModal(usuarioId, rolPersona, inmuebleId);
};


/* Importación de libros XLSX: selección explícita de hoja. */
window.initImportView = function () {
    const input = document.getElementById('excelFile'); const zona = document.getElementById('uploadZone'); const seccion = document.getElementById('mappingSection'); const formulario = document.getElementById('mappingForm');
    if (!input || !zona || !seccion || !formulario || zona.dataset.importacionConHojas) return;
    zona.dataset.importacionConHojas = '1'; input.accept = '.xlsx,.csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv'; zona.querySelector('h3').textContent = 'Arrastra un archivo XLSX o CSV aquí';
    const tipos = {
        residentes: { titulo: 'Residentes', descripcion: 'Documento, nombre, correo, teléfono y unidad se mapean desde cualquier formato.', campos: [['documento', 'Documento *'], ['nombre', 'Nombre completo *'], ['email', 'Correo'], ['contacto', 'Teléfono'], ['inmueble_nomenclatura', 'Nomenclatura del inmueble']] },
        propietarios: { titulo: 'Propietarios', descripcion: 'Documento, nombre, correo, teléfono y unidad se mapean desde cualquier formato.', campos: [['documento', 'Documento *'], ['nombre', 'Nombre completo *'], ['email', 'Correo'], ['contacto', 'Teléfono'], ['inmueble_nomenclatura', 'Nomenclatura del inmueble']] },
        inmuebles: { titulo: 'Apartamentos o casas', descripcion: 'Mapea los datos disponibles de las unidades.', campos: [['nomenclatura', 'Nomenclatura *'], ['tipo_unidad', 'Tipo: apartamento/casa'], ['torre', 'Torre o bloque'], ['apartamento', 'Apartamento'], ['coeficiente', 'Coeficiente'], ['cuota_administracion', 'Cuota de administración'], ['mora_actual', 'Mora actual']] },
        parqueaderos: { titulo: 'Parqueaderos, motos y bodegas', descripcion: 'Mapea código, clase física y unidad asignada si aplica.', campos: [['codigo', 'Código *'], ['tipo', 'Régimen'], ['clase_espacio', 'Clase: carro/moto/bodega'], ['sotano', 'Sótano o nivel'], ['ubicacion', 'Ubicación'], ['estado', 'Estado'], ['observaciones', 'Observaciones'], ['inmueble_nomenclatura', 'Nomenclatura asignada']] },
        mascotas: { titulo: 'Mascotas', descripcion: 'Mapea unidad, tipo, nombre, raza y descripción.', campos: [['inmueble_nomenclatura', 'Nomenclatura del inmueble *'], ['tipo_mascota', 'Tipo *'], ['nombre_mascota', 'Nombre *'], ['raza', 'Raza'], ['descripcion', 'Descripción']] }
    };
    zona.insertAdjacentHTML('beforebegin', `<p class="import-help">1. Elige la información. 2. Sube un XLSX o CSV. 3. Si es XLSX, selecciona la hoja a importar. 4. Mapea las columnas y valida antes de guardar.</p><div class="import-type-picker">${Object.entries(tipos).map(([clave, tipo], indice) => `<label><input type="radio" name="importTipo" value="${clave}" ${indice === 0 ? 'checked' : ''}><strong>${tipo.titulo}</strong><small>${tipo.descripcion}</small></label>`).join('')}</div><div id="importSheetPicker" class="import-sheet-picker hidden"></div><div id="importPreview"></div>`);
    let headers = []; let listo = false;
    const seleccionado = () => document.querySelector('input[name="importTipo"]:checked')?.value || 'residentes';
    const indiceHoja = () => document.getElementById('importSheetIndex')?.value || '0';
    const sinonimos = { documento: ['documento', 'cedula', 'identificacion', 'id'], nombre: ['nombre', 'nombrecompleto', 'nombres'], email: ['email', 'correo', 'mail'], contacto: ['telefono', 'celular', 'contacto'], inmueble_nomenclatura: ['inmueble', 'nomenclatura', 'apartamento', 'apto', 'unidad', 'casa'], tipo_mascota: ['tipo', 'especie', 'tipomascota'], nombre_mascota: ['nombre', 'mascota', 'nombremascota'], tipo_unidad: ['tipounidad', 'tipo'], cuota_administracion: ['cuota', 'administracion'], mora_actual: ['mora', 'deuda'] };
    const opciones = campo => `<option value="">— No mapear —</option>${headers.map((header, indice) => { const normalizado = normalizarImportacion(header); const sugerido = (sinonimos[campo] || [campo]).map(normalizarImportacion).includes(normalizado) || normalizado.includes(normalizarImportacion(campo)); return `<option value="${indice}" ${sugerido ? 'selected' : ''}>${escapeHtml(header || `Columna ${indice + 1}`)}</option>`; }).join('')}`;
    const mapeo = () => Object.fromEntries(tipos[seleccionado()].campos.map(([campo]) => [campo, formulario.querySelector(`[name="map_${campo}"]`)?.value || '']));
    const resetearMapeo = () => { headers = []; listo = false; seccion.classList.add('hidden'); formulario.innerHTML = ''; document.getElementById('accionesMapeo')?.remove(); document.getElementById('importPreview').innerHTML = ''; document.getElementById('btnProcesarImportacion').disabled = true; };
    const enviar = async accion => { const archivo = input.files[0]; if (!archivo) return { status: 'error', message: 'Selecciona un archivo XLSX o CSV primero.' }; const datos = new FormData(); datos.append('action', accion); datos.append('tipo', seleccionado()); datos.append('sheet_index', indiceHoja()); datos.append('mapping', JSON.stringify(mapeo())); datos.append('file', archivo); return leerRespuestaImportacion(await fetch('api/import.php', { method: 'POST', body: datos })); };
    const cargarCabeceras = async () => { resetearMapeo(); const archivo = input.files[0]; if (!archivo) return; const datos = new FormData(); datos.append('action', 'get_headers'); datos.append('tipo', seleccionado()); datos.append('sheet_index', indiceHoja()); datos.append('file', archivo); const resultado = await leerRespuestaImportacion(await fetch('api/import.php', { method: 'POST', body: datos })); if (resultado.status !== 'success') return window.notificar(resultado.message, 'error'); headers = resultado.data.headers; formulario.innerHTML = tipos[seleccionado()].campos.map(([campo, etiqueta]) => `<div class="mapping-item"><label>${etiqueta}</label><select name="map_${campo}">${opciones(campo)}</select></div>`).join(''); seccion.classList.remove('hidden'); seccion.querySelector('h3').textContent = `Asignar columnas: ${tipos[seleccionado()].titulo}`; seccion.querySelector('p').textContent = `${resultado.data.rows} fila(s) detectadas en la hoja seleccionada. Los campos con * son obligatorios.`; formulario.insertAdjacentHTML('afterend', '<div class="actions mt-4" id="accionesMapeo"><button type="button" class="btn btn-ghost" id="btnPrevisualizarImportacion">Validar y ver vista previa</button></div>'); document.getElementById('btnPrevisualizarImportacion').onclick = async () => { const previo = await enviar('preview'); const panel = document.getElementById('importPreview'); if (previo.status !== 'success') return window.notificar(previo.message, 'error'); const info = previo.data; listo = info.summary.errores === 0 && info.summary.validas > 0; panel.innerHTML = `<p class="${listo ? 'import-ok' : 'import-errors'}">${listo ? `Listo: ${info.summary.validas} fila(s) válidas.` : `${info.summary.errores} error(es). Corrígelos antes de importar.`}</p>${info.sample?.length ? `<div class="import-preview-wrap"><table class="import-preview-table"><thead><tr>${Object.keys(info.sample[0]).filter(campo => campo !== '_fila').map(campo => `<th>${escapeHtml(campo)}</th>`).join('')}</tr></thead><tbody>${info.sample.map(fila => `<tr>${Object.keys(fila).filter(campo => campo !== '_fila').map(campo => `<td>${escapeHtml(fila[campo] || '—')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>` : ''}${info.errors?.length ? `<div class="import-errors">${info.errors.map(error => `Fila ${error.fila}: ${escapeHtml(error.mensaje)}`).join('<br>')}</div>` : ''}`; document.getElementById('btnProcesarImportacion').disabled = !listo; }; };
    input.onchange = async () => { const archivo = input.files[0]; if (!archivo) return; resetearMapeo(); const picker = document.getElementById('importSheetPicker'); picker.classList.add('hidden'); const datos = new FormData(); datos.append('action', 'get_sheets'); datos.append('tipo', seleccionado()); datos.append('file', archivo); const resultado = await leerRespuestaImportacion(await fetch('api/import.php', { method: 'POST', body: datos })); if (resultado.status !== 'success') return window.notificar(resultado.message, 'error'); const hojas = resultado.data.sheets || []; picker.innerHTML = `<label>Hoja a importar<select id="importSheetIndex">${hojas.map(hoja => `<option value="${Number(hoja.indice)}">${escapeHtml(hoja.nombre)}${hoja.oculta ? ' (oculta)' : ''}</option>`).join('')}</select><small>Las cabeceras y filas se leerán únicamente de esta hoja.</small></label>`; picker.classList.remove('hidden'); document.getElementById('importSheetIndex').onchange = cargarCabeceras; await cargarCabeceras(); };
    document.querySelectorAll('input[name="importTipo"]').forEach(radio => radio.onchange = () => { input.value = ''; resetearMapeo(); const picker = document.getElementById('importSheetPicker'); picker.innerHTML = ''; picker.classList.add('hidden'); });
    document.getElementById('btnProcesarImportacion').onclick = async () => { if (!listo) return window.notificar('Valida la vista previa sin errores antes de importar.', 'warning'); if (!await confirmarAccion({ titulo: '¿Importar datos?', texto: 'Se crearán o actualizarán únicamente las filas validadas de la hoja seleccionada.', confirmar: 'Importar datos', icono: 'question' })) return; const boton = document.getElementById('btnProcesarImportacion'); boton.disabled = true; const original = boton.textContent; boton.textContent = 'Importando…'; try { const resultado = await enviar('process'); window.notificar(resultado.message, resultado.status === 'success' ? 'success' : 'error'); if (resultado.status === 'success') { input.value = ''; resetearMapeo(); document.getElementById('importSheetPicker').innerHTML = ''; document.getElementById('importSheetPicker').classList.add('hidden'); } } finally { boton.textContent = original; boton.disabled = false; } };
};


/* Bandeja administrativa y ciclo de seguimiento para novedades de vigilancia. */
function etiquetaEstadoNovedad(estado) { return ({ pendiente: 'Pendiente', en_progreso: 'En progreso', resuelta: 'Resuelta', cerrada: 'Cerrada' })[estado] || estado; }
function iconoSeguimientoNovedad(tipo) { return ({ creacion: 'fa-flag', vista: 'fa-eye', respuesta: 'fa-reply', cambio_estado: 'fa-arrow-right-arrow-left', resolucion: 'fa-circle-check', cierre: 'fa-lock', reapertura: 'fa-rotate-left' })[tipo] || 'fa-comment'; }

async function cargarNovedadesVigilancia() {
    const tabla = document.getElementById('tb-novedades-vigilancia');
    if (!tabla) return;
    const estado = document.getElementById('filtroEstadoNovedad')?.value || '';
    const noVistas = document.getElementById('filtroNoVistasNovedad')?.checked ? '1' : '';
    const buscar = document.getElementById('buscarNovedadVigilancia')?.value.trim() || '';
    const parametros = new URLSearchParams({ action: 'list' }); if (estado) parametros.set('estado', estado); if (noVistas) parametros.set('solo_no_vistas', noVistas); if (buscar) parametros.set('buscar', buscar);
    const resultado = await (await fetch(`api/novedades.php?${parametros}`)).json();
    if (resultado.status !== 'success') return window.notificar(resultado.message || 'No fue posible cargar las novedades.', 'error');
    const novedades = resultado.data || [];
    const resumen = document.getElementById('resumenNovedadesVigilancia');
    if (resumen) resumen.innerHTML = ['pendiente', 'en_progreso', 'resuelta', 'cerrada'].map(estadoActual => `<article class="novedad-summary-card estado-${estadoActual}"><span>${etiquetaEstadoNovedad(estadoActual)}</span><strong>${novedades.filter(item => item.estado === estadoActual).length}</strong></article>`).join('') + `<article class="novedad-summary-card no-vistas"><span>No vistas por mí</span><strong>${novedades.filter(item => !item.mi_vista_en).length}</strong></article>`;
    tabla.innerHTML = novedades.length ? novedades.map(item => `<tr class="novedad-row ${item.mi_vista_en ? '' : 'is-unread'}" onclick="window.abrirDetalleNovedadVigilancia(${Number(item.id)})"><td>${item.mi_vista_en ? '' : '<span class="novedad-unread-dot" title="Aún no vista"></span>'}${escapeHtml(item.fecha_operativa || item.fecha_registro)}</td><td><strong>${escapeHtml(item.asunto)}</strong><br><small>${escapeHtml(String(item.novedad || '').slice(0, 90))}${String(item.novedad || '').length > 90 ? '…' : ''}</small></td><td>${escapeHtml(item.vigilante)}</td><td><span class="reserva-estado estado-${escapeHtml(item.estado)}">${escapeHtml(etiquetaEstadoNovedad(item.estado))}</span></td><td>${escapeHtml(item.ultimo_seguimiento || 'Sin seguimiento')}</td><td class="novedad-actions">${item.mi_vista_en ? '<span class="muted">Vista</span>' : `<button class="btn btn-ghost" type="button" onclick="event.stopPropagation(); window.marcarNovedadVigilanciaVista(${Number(item.id)})"><i class="fa-solid fa-eye"></i> Marcar vista</button>`}<button class="btn btn-ghost" type="button" onclick="event.stopPropagation(); window.abrirDetalleNovedadVigilancia(${Number(item.id)})">Gestionar</button></td></tr>`).join('') : '<tr><td colspan="6" class="empty-state">No hay novedades para este filtro.</td></tr>';
}

window.marcarNovedadVigilanciaVista = async function (novedadId) {
    const datos = new FormData(); datos.append('action', 'marcar_vista'); datos.append('novedad_id', novedadId);
    const resultado = await (await fetch('api/novedades.php', { method: 'POST', body: datos })).json();
    window.notificar(resultado.message, resultado.status === 'success' ? 'success' : 'error');
    if (resultado.status === 'success') cargarNovedadesVigilancia();
};

window.abrirDetalleNovedadVigilancia = async function (novedadId) {
    const resultado = await (await fetch(`api/novedades.php?action=detalle&novedad_id=${encodeURIComponent(novedadId)}`)).json();
    if (resultado.status !== 'success') return window.notificar(resultado.message || 'No fue posible abrir la novedad.', 'error');
    const { novedad, adjuntos = [], seguimientos = [] } = resultado.data;
    let modal = document.getElementById('modalDetalleNovedadVigilancia');
    if (!modal) { document.body.insertAdjacentHTML('beforeend', '<div id="modalDetalleNovedadVigilancia" class="login-modal novedad-detail-modal hidden"></div>'); modal = document.getElementById('modalDetalleNovedadVigilancia'); }
    const archivos = adjuntos.length ? adjuntos.map(adjunto => `<a href="api/porteria.php?action=ver_adjunto_novedad&adjunto_id=${Number(adjunto.id)}" target="_blank" rel="noopener"><i class="fa-solid fa-paperclip"></i> ${escapeHtml(adjunto.nombre_original)}</a>`).join('') : '<span class="muted">Sin adjuntos.</span>';
    const cronologia = seguimientos.length ? seguimientos.map(evento => `<article class="novedad-timeline-item tipo-${escapeHtml(evento.tipo)}"><i class="fa-solid ${iconoSeguimientoNovedad(evento.tipo)}"></i><div><header><strong>${escapeHtml(({ creacion: 'Reporte inicial', vista: 'Vista por Administración', respuesta: 'Respuesta de Administración', cambio_estado: 'Cambio de estado', resolucion: 'Novedad resuelta', cierre: 'Novedad cerrada', reapertura: 'Novedad reabierta' })[evento.tipo] || evento.tipo)}</strong><small>${escapeHtml(evento.autor || 'Sistema')} · ${escapeHtml(evento.creado_en)}</small></header>${evento.contenido ? `<p>${escapeHtml(evento.contenido)}</p>` : ''}${evento.estado_nuevo ? `<span class="reserva-estado estado-${escapeHtml(evento.estado_nuevo)}">${escapeHtml(etiquetaEstadoNovedad(evento.estado_anterior || ''))} → ${escapeHtml(etiquetaEstadoNovedad(evento.estado_nuevo))}</span>` : ''}</div></article>`).join('') : '<p class="muted">Sin eventos registrados.</p>';
    modal.innerHTML = `<div class="login-box novedad-detail-dialog"><button class="close-btn" type="button" onclick="document.getElementById('modalDetalleNovedadVigilancia').classList.add('hidden')"><i class="fa-solid fa-xmark"></i></button><header class="novedad-detail-heading"><div><p class="section-kicker">Novedad de vigilancia</p><h2>${escapeHtml(novedad.asunto)}</h2><p>Reportada por <strong>${escapeHtml(novedad.vigilante)}</strong> · ${escapeHtml(novedad.fecha_novedad || novedad.fecha_registro)}</p></div><span class="reserva-estado estado-${escapeHtml(novedad.estado)}">${escapeHtml(etiquetaEstadoNovedad(novedad.estado))}</span></header><section class="novedad-original"><h3>Reporte del vigilante</h3><p>${escapeHtml(novedad.novedad)}</p><div class="novedad-files">${archivos}</div></section><section class="novedad-timeline"><h3>Seguimiento e historial</h3>${cronologia}</section><form id="formSeguimientoNovedad" class="novedad-followup-form"><h3>Responder o actualizar estado</h3><textarea name="contenido" maxlength="10000" placeholder="Respuesta, gestión realizada o solución aplicada"></textarea><label>Estado<select name="estado"><option value="pendiente" ${novedad.estado === 'pendiente' ? 'selected' : ''}>Pendiente</option><option value="en_progreso" ${novedad.estado === 'en_progreso' ? 'selected' : ''}>En progreso</option><option value="resuelta" ${novedad.estado === 'resuelta' ? 'selected' : ''}>Resuelta</option><option value="cerrada" ${novedad.estado === 'cerrada' ? 'selected' : ''}>Cerrada</option></select><small>Para resolver o cerrar debes describir la solución.</small></label><button class="btn btn-primary" type="submit">Guardar seguimiento</button></form></div>`;
    const form = modal.querySelector('#formSeguimientoNovedad');
    form.onsubmit = async evento => { evento.preventDefault(); const datos = new FormData(form); const contenido = String(datos.get('contenido') || '').trim(); const estado = String(datos.get('estado') || ''); if (estado === novedad.estado && !contenido) return window.notificar('Escribe una respuesta o selecciona un nuevo estado.', 'warning'); datos.append('novedad_id', novedadId); if (estado === novedad.estado) { datos.set('action', 'agregar_seguimiento'); } else { datos.set('action', 'actualizar_estado'); datos.set('comentario', contenido); datos.delete('contenido'); } const respuesta = await fetch('api/novedades.php', { method: 'POST', body: datos }); const resultadoSeguimiento = await respuesta.json(); window.notificar(resultadoSeguimiento.message, resultadoSeguimiento.status === 'success' ? 'success' : 'error'); if (resultadoSeguimiento.status === 'success') { await cargarNovedadesVigilancia(); window.abrirDetalleNovedadVigilancia(novedadId); } };
    modal.classList.remove('hidden');
    cargarNovedadesVigilancia();
};

const cargarVistaNovedadesVigilanciaBase = loadView;
loadView = function (vista) {
    if (vista !== 'novedades-vigilancia') return cargarVistaNovedadesVigilanciaBase(vista);
    if (currentUser?.rol !== 'admin') return window.notificar('Solo administración puede gestionar las novedades de vigilancia.', 'error');
    document.getElementById('view-container').innerHTML = `<div class="view novedades-vigilancia-view"><div class="section-toolbar"><div><p class="section-kicker">Vigilancia · Administración</p><h1 class="page-title">Seguimiento de novedades</h1><p class="muted">Revisa reportes de vigilancia, responde, registra la gestión y deja constancia de la solución o cierre.</p></div></div><section id="resumenNovedadesVigilancia" class="novedad-summary-grid"><span class="muted">Cargando resumen…</span></section><section class="card novedades-vigilancia-card"><div class="novedad-toolbar"><div><h3>Bandeja de novedades</h3><p class="muted">Abrir una novedad la marca como vista para tu usuario. El historial no se puede borrar.</p></div><div class="novedad-filters"><input id="buscarNovedadVigilancia" type="search" placeholder="Buscar asunto, reporte o vigilante"><select id="filtroEstadoNovedad"><option value="">Todos los estados</option><option value="pendiente">Pendientes</option><option value="en_progreso">En progreso</option><option value="resuelta">Resueltas</option><option value="cerrada">Cerradas</option></select><label><input id="filtroNoVistasNovedad" type="checkbox"> Solo no vistas</label></div></div><div class="table-responsive"><table class="data-table"><thead><tr><th>Fecha</th><th>Reporte</th><th>Vigilante</th><th>Estado</th><th>Último seguimiento</th><th>Acciones</th></tr></thead><tbody id="tb-novedades-vigilancia"><tr><td colspan="6">Cargando…</td></tr></tbody></table></div></section></div>`;
