...
{
    $stmt = $pdo->prepare('SELECT id, estado FROM parqueaderos WHERE id = ? AND conjunto_id = ? FOR UPDATE');
...
if ($action === 'historial') {
    ...
    $stmt = $pdo->prepare("SELECT a.asignado_en, a.retirado_en, a.motivo_retiro, i.torre, i.nomenclatura, i.apartamento FROM asignaciones_parqueadero a JOIN parqueaderos p ON p.id = a.parqueadero_id JOIN inmuebles i ON i.id = a.inmueble_id WHERE a.parqueadero_id = ? AND p.conjunto_id = ? ORDER BY a.asignado_en DESC");
...
if ($action === 'guardar') {
    ...
    if ($codigo === '' || !in_array($tipo, ['privado', 'administracion', 'visitante', 'otro'], true) || !in_array($estado, ['disponible', 'inactivo'], true)) responseJSON('error', 'Completa código, tipo y estado válidos');
    if (mb_strlen($codigo) > 50 || mb_strlen($observaciones) > 255) responseJSON('error', 'La información excede la longitud permitida');
    if ($id) {
        ...
        if ($duplicado->fetch()) responseJSON('error', 'Ya existe un parqueadero con ese código');
        $stmt = $pdo->prepare("UPDATE parqueaderos SET codigo = ?, tipo = ?, estado = CASE WHEN estado = 'asignado' THEN 'asignado' ELSE ? END, observaciones = ? WHERE id = ? AND conjunto_id = ?");
        $stmt->execute([$codigo, $tipo, $estado, $observaciones ?: null, $id, $conjuntoId]);
        responseJSON('success', 'Parqueadero actualizado');
    }
    $duplicado = $pdo->prepare('SELECT id FROM parqueaderos WHERE conjunto_id = ? AND codigo = ?');
    $duplicado->execute([$conjuntoId, $codigo]);
    if ($duplicado->fetch()) responseJSON('error', 'Ya existe un parqueadero con ese código');
    $stmt = $pdo->prepare('INSERT INTO parqueaderos (conjunto_id, codigo, tipo, estado, observaciones) VALUES (?, ?, ?, ?, ?)');
    $stmt->execute([$conjuntoId, $codigo, $tipo, $estado, $observaciones ?: null]);
    responseJSON('success', 'Parqueadero creado');
}
if ($action === 'asignar') {
