...
if ($action === 'list_comunicados') {
    $stmt = $pdo->query("SELECT c.*, u.nombre as autor FROM comunicados c JOIN usuarios u ON c.autor_id = u.id ORDER BY c.fecha_publicacion DESC LIMIT 20");
    responseJSON('success', '', $stmt->fetchAll());
}
elseif ($action === 'crear_comunicado') {
    ...
    $stmt = $pdo->prepare("INSERT INTO comunicados (titulo, contenido, autor_id) VALUES (?, ?, ?)");
...
elseif ($action === 'list_auditoria') {
    if ($rol !== 'admin') responseJSON('error', 'Sin permisos');
    $stmt = $pdo->query("SELECT a.*, u.nombre as usuario FROM auditoria_logs a LEFT JOIN usuarios u ON a.usuario_id = u.id ORDER BY a.fecha DESC LIMIT 50");
    responseJSON('success', '', $stmt->fetchAll());
}
?>
